...
| I-02 | After every successful roll-up for `(household, carer, week_start)`, `timesheets.total_minutes` equals `sumWorkedMinutes` over every completed entry whose `local_date` is in that week window as returned by `listForCarerWeek`. | Hours | `timesheetCommandService.ts` (derive-on-write); still **NOWHERE** as a cross-table DB constraint, but **drift is now detected in production**: `056_integrity_checks.sql:91` check 1 `timesheet_total_mismatch` recomputes the total from the entries daily at 04:10 and fails the run | `migration056IntegrityChecks.test.ts` › `emits "timesheet_total_mismatch"`; `mirrors computeWorkedMinutes: rounded span minus break, floored at zero`; `counts only completed entries, like sumWorkedMinutes`. Behaviourally proven: a seeded mismatch fires it on the local stack | No. **Drift is no longer undetectable** — the first production run found two orphan-total timesheets and a 3-minute drift (`OPEN-ITEMS.md` → 2026-08-06) |
| I-03 | `WeekEarnings.worked_minutes` for a week equals `sumWorkedMinutes` of the same completed worked/`manual_adjustment` entries the engine received (same formula as roll-up). | Hours / Pay | **Structurally enforced now**: `entryMinutes()` (`domains/timesheet/utils/workedMinutes.ts:59`) is the single source, and its *only* two callers are `sumWorkedMinutes` (`:87`, the roll-up path) and `weekEarningsService.ts:277` (the pricing path). Totals and pay read the same function, so they cannot disagree by construction. `061` is the SQL twin | `workedMinutes.test.ts` › `banks the fragment residual, not its own re-rounded span`; `never banks a negative, even if a stored figure went bad`; `ignores scheduled_minutes entirely for worked and manual_adjustment kinds`. Cross-checked in SQL by `056` › `honours C7: scheduled_minutes is authoritative for cancellation pay` | No. **Bridge no longer untested** — it is now one function with two callers rather than two formulas |
...
| I-05 | No two completed entries for the same carer have overlapping half-open clock intervals `[in,out)`. | Hours | App: `assertNoOverlap` queries by clock instant, not week. **DB backstop now exists**: `055_time_entry_overlap_exclusion.sql:118` (`time_entries_no_clock_overlap_per_carer`, carer-global) and `:134` (`…_per_household`), both `EXCLUDE USING gist`. **Production canary**: `056:212` check 6 `entry_overlap` | `migration055TimeEntryOverlapExclusion.test.ts` (21 cases); `056` › `entry_overlap reproduces 055’s two permission rules`; cross-Monday span tests in `timesheetCommandService.test.ts` | No — F-B1-1 closed. **Residual:** both constraints are partial on `clock_out_at is not null`, so a completed span overlapping an open *running* session is refused by app code alone |
| I-06 | Every minute of unique wall-clock work appears in at most one `(carer, week_start)` timesheet total. | Hours / Week filing | Relies on I-05 + week bucketing. Strengthened in round 2: a week-crossing session is now **split at household-local Monday midnight** (`clockOutAcrossWeeks`, `timesheetCommandService.ts:641-729`), so its minutes are divided between the two weeks rather than filed whole into one | `TimesheetCommandService.clockOut — a session that crosses Monday splits (C6)` › `conserves worked minutes and the break across the split`; `rolls up BOTH weeks, not just the clock-in week`; plus `056` check 6 as the production canary | No — F-B1-1 closed, and C6 (the coarse-filing ceiling that replaced it) closed too |
...
| I-11 | Mobile `computeWorkedMinutesFromInstants(in, outMs, break)` equals API `computeWorkedMinutes(in, outIso, break)` for every shared golden `(in, out, break)`. | Hours | Partly enforced. Mobile `computeEntryMinutes` (`apps/mobile/src/domains/timesheet/utils/entryMinutes.ts:54`) now **mirrors the API's cancellation branch** — `scheduled_minutes` is authoritative for a `cancellation_paid` row (`:56-58`), reached through one guard shared by five call sites, matching `workedMinutes.ts:63`. The span formulas are still two implementations reconciled in prose, not in code | Mirrored tests on both sides (`computeEntryMinutes — cancellation pay banks its stored minutes`; `matches computeEntryMinutes for the same inputs (same underlying rule)`), but **still no shared golden vector file** — B10#6 stands | No confirmed cent bug. **Parity still unenforced** structurally, but the cancellation branch is now a true mirror on both sides, clamp included — `Math.max(0, entry.scheduled_minutes)` at `workedMinutes.ts:74` and `entryMinutes.ts:60`, with a `MOBILE TWIN:` note at `workedMinutes.ts:59` naming the counterpart and bounding the twinning to the cancellation branch (mobile deliberately shows elapsed time for a running entry where the server returns null). The clamp asymmetry found during this pass is closed; the *span* formulas are still two implementations with no shared vectors |
...
| I-22 | Approve CAS: `UPDATE … WHERE status='submitted' AND updated_at=:read_version` freezes `gross_minor`/`currency`/`earnings`/`earnings_computed_at` from `computeForWeek` at that instant. | Freeze / Idempotency | `timesheetRepository.ts`; `timesheetCommandService.ts` (approve path) | CAS refusal tests in `timesheetCommandService.test.ts` / repo; **approve always mocks engine** — F-B10-4 | No for CAS; **end-to-end freeze≠engine still untested.** Round 2 did not touch this — F-B10-4 remains STILL OPEN in `OPEN-ITEMS.md`, and every `computeForWeek` in the approve tests is still an injected stub |
...
| I-33 | Concurrent or sequential delta writes for one `(household, time_off)` never leave net paid minutes ≠ last intended total (serialization / single-writer). | PTO / Idempotency | `050_serialise_pto_corrections.sql:55` (`apply_pto_correction`, anchor `FOR UPDATE` lock + CAS); both the first marking and every correction go through it. **Production canary**: `056:136` check 3 `pto_net_negative` — a ledger that has netted below zero is the visible symptom of a lost update | `migration050SerialisePtoCorrections.test.ts`; `ptoCommandService.test.ts` › `a correction that loses the race re-reads and applies its OWN delta — no lost update`; `056` › `pto_net_negative reads the ledger’s sign convention the right way round` | No — F-B4-2 closed |
...
| I-38 | `shifts.cancellation_paid = true` if and only if a `time_entries` row `kind=cancellation_paid` for that shift exists and is included in the household-tz week total/earnings. | State / Pay | Accept rethrows instead of swallowing the recorder failure; `cancellationPayReconcileJob` sweeps hourly (`054`) and re-calls the idempotent recorder. **Production canary**: `056:185` check 5 `cancellation_unsettled` — a shift flagged paid with no entry behind it after a 2-hour grace, deliberately outside the hourly reconcile window so a mid-flight repair is not reported as a violation | `shiftChangeRequestCommandService.pushes.test.ts` › `fails the accept when the paid-cancel entry cannot be written`; `cancellationPayReconcileJob.test.ts` › `creates the missing payable entry for a stuck shift`; `056` › `cancellation_unsettled waits 2 hours, outside the hourly reconcile window` | No — F-B2-5 / F-B5-1 / F-B9-1 closed. **Residual (C8):** nothing checks the *opposite* direction — Σ fragment minutes is never compared against the booked span, so a settled over-bank is invisible |
...
| I-45 | Money tables `pay_arrangements` / `pto_ledger` / `expenses` have select-only RLS; helpers and other carers cannot SELECT another carer’s rows via PostgREST. | Authz / RLS | SQL policies in `041`/`043`/`044`; API uses service role (bypasses RLS) | **Now tested at runtime.** `apps/api/tests/integration/rls.integration.test.ts` connects with a real nanny JWT: `N1 cannot SELECT household 2's PTO ledger`; `N1 cannot SELECT household 2's expenses`; `N1 cannot SELECT household 2's time entries`. Run in CI (`ci.yml:296-300`) against a database `supabase db reset --local` has just built from all 61 migrations, and proven discriminating against weakened policy copies | No. **F-B10-2 closed** — this row moved from "untested at runtime" to live evidence. The twelve assertions cover the policies this audit touched, not the whole policy surface |
...
> **Written before the fix work; most of it has since been done.** The cheapest-fix
> suggestions below were largely taken up — `055` shipped the `EXCLUDE USING gist`
> proposed for I-05/I-06 (1), `050` the advisory-lock RPC for I-33 (8), `051` both the
> atomic expense/freeze write (9) and the pending-claim unique index (10), `049`/`052`
> the write lockdown for I-43 (16), the CAS-in-one-RPC for I-44 (17), and round 2
> shipped the single-implementation `entryMinutes()` for I-03 (6) and the live-JWT RLS
> test for I-45 (18). Per-row status is in the register above and in
> [`CLOSURE-TABLE.md`](./CLOSURE-TABLE.md). **Still genuinely open:** I-11's shared
> golden vectors (5), I-18's cross-implementation vector (7), I-37's API amount cap
> (11), and I-22's end-to-end freeze test (19). The list is kept as filed rather than
> rewritten, because the reasoning for each cheapest fix is what made it cheap.
...
> **All five are closed** across rounds 1–2, and four of them now carry a daily production
> canary in `056_integrity_checks.sql` on top of the fix: I-38 → `cancellation_unsettled`,
> I-05/I-06 → `entry_overlap`, I-07/I-08 → `timesheet_total_mismatch` + `orphan_week`,
> I-33 → `pto_net_negative`. **I-34 is the exception** — `051`'s locked RPC closed the
> expense-vs-freeze race, but no check watches for a recurrence.
> Kept as written: this is the ranking that decided what got fixed first, and it held up.
