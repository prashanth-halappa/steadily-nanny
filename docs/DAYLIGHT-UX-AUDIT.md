...
> **Status — 2026-08-03, residual pass re-verified against source.** `bun run qc` green (1090 mobile + 570 API). The residual pass claimed #12, #14, #24, #30, #31, #32 and #40 all closed. Independent check confirms **six of the seven**; #24 is overstated. Both dead-code items are genuinely gone.
>
> **Verified fixed: #5, #6, #8–#44 — except #7 and #24.** The residual items are real, not cosmetic:
>
> | # | Confirmed by |
> |---|---|
> | **#12** | `SchedulePendingScreen.tsx:274` renders `pending.acceptedMeansShifts` — *"Your week is accepted. Individual shifts may still show as Pending on the calendar until each one is confirmed."* That is the sentence the two status ladders were missing. `formatWeekRangeLabel` now gives the pill a date. |
> | **#14** | `StatusPill` now rendered (5 refs); `detail.shortNoticePaidHint` at `:204` and `detail.awaitingCoParent` at `:345` are finally wired; change-request timestamps via `formatInstantDisplay(req.created_at, …)` at `:338`. |
> | **#30** | `getWeekdayOrder(profile.data?.week_starts_on)` at `:125` reorders `days` before render, so the nanny reads the parent's week. The total and both actions now sit in a footer **outside** the `ScrollView` (`</ScrollView>` `:224`, footer `:227`, `elevation.row` inline) — always visible. |
> | **#31 / #33** | `CoverageLanesView.tsx` deleted outright, taking its `bg-muted` row surface with it. |
> | **#32** | `formatDisplayDate` in `shiftGrouping.ts:37` and `ShiftDetailScreen.tsx:182`; the IANA zone is now conditional on `showShiftZone`, i.e. shown only when it differs from the reader's — exactly what the finding asked. |
> | **#40** | `CardContent`'s `pt-0` is gone; only `CardFooter` (`card.tsx:108`) keeps it, which is correct. |
>
...
> | # | Gap |
> |---|---|
> | **#24** | *Partial, claimed closed.* Chevrons, the external-link glyph for the legal pair, and 44pt touch targets all landed. **Row grouping did not** — `settings.tsx` has zero `<Card`, `rounded-row`, `elevation.row` or `useElevation` matches, so the rows are still an ungrouped column separated by nothing. The finding asked for grouping *by light*; only the affordance half shipped. |
> | **#14** | *Residual.* The change-request block now shows status, timestamp and whose turn it is, but still never names **who raised the request**. |
> | **#7** | No forgotten-clock-out guard anywhere in `domains/today` or the API timesheet service. Deferred. |
> | **P0-2** | No correction path after clock-out. Deferred. |
>
> **Constraints re-checked across the residual pass, all clean:** no `className` on any Reanimated `Animated.View`, no Tailwind `shadow-*`, no `fontFamily`, no new bare `<Modal>`, and `en`/`es` at full key parity in every namespace.
...
| 24 | **Settings is a column of text links.** Rows are `AnimatedPressable` wrapping a bare `<Body>` with no padding — **~24pt against a 44pt minimum**, on nine rows, while `spacing.minTouchTarget: 44` sits unused two files away. No chevrons, so nothing distinguishes "Children" (navigates) from "Privacy Policy" (leaves the app). An orphan group at `:173-180` renders the legal pair with no `<H4>`. Language outranks Account. Nothing states who is signed in, in which role — on the screen where you delete your account. `settings.notifications` is defined in both locales and rendered nowhere. ⚠️ **STILL PARTIAL (re-verified 2026-08-03)** — chevrons, external-link glyph and 44pt targets landed; **row grouping did not** (zero `<Card` / `rounded-row` / `elevation.row` in `settings.tsx`). | `settings.tsx` | M |
