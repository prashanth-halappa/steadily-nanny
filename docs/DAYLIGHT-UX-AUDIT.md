...
> **Status — 2026-08-03, after independent verification.** `bun run qc` green (1081 mobile + 570 API). The closure pass claimed #5–#6 and #8–#44 were all addressed; verification against the source found **31 genuinely fixed, 2 partial, and 5 not fixed**. Corrected below and marked inline.
>
> **Verified fixed (31):** #5, #6, #8, #9, #10, #11, #13, #15, #16, #17, #18, #19, #20, #21, #22, #23, #25, #26, #27, #28, #29, #33, #34, #35, #36, #37, #38, #39, #41, #43, #44 — plus P0-4's dialog-layout item, which the pass fixed and the previous status line wrongly still listed as open (`alert-dialog.tsx:107` is now `flex flex-col sm:justify-end gap-2`).
>
> **Still open — the gaps:**
>
> | # | Gap | Why it survived |
> |---|---|---|
> | **#14** | Shift detail still has no status, no actor, no timestamp, no consequence; `detail.awaitingCoParent` / `detail.shortNoticePaidHint` still render nowhere; times are still free-text `HH:MM` | `ShiftDetailScreen.tsx` was never opened |
> | **#12** | Week-level **Accepted** vs day-level **Pending** still unreconciled; the pill still has no subject, actor or date | `SchedulePendingScreen.tsx` was never opened |
> | **#32** | Raw ISO dates still parent-facing: `shiftGrouping.ts:36` (agenda headers, `Thursday · 2026-07-30`) and `ShiftDetailScreen.tsx:142` (`{shift.local_date} · {shift.timezone}`) | both files untouched |
> | **#30** | Nanny still reads the week in raw API order — `ScheduleRespondScreen.tsx:173` is a bare `days.map`, no `getWeekdayOrder`, so she sees a different week than the parent sent | only the chip-overflow half of this row was done |
> | **#40** | `CardContent` is still `p-5.5 pt-0` (`card.tsx:95`), so header-less cards remain flush at the top | not attempted |
> | **#24** | *Partial* — touch targets fixed (`spacing.minTouchTarget` applied), but still no chevrons, no external-link glyph on the legal pair, no row grouping | affordance half not done |
> | **#31** | *Partial* — `TimeEntryDayRow` `border-b` and `AgendaView` `bg-muted` both fixed; `CoverageLanesView.tsx:98` still `bg-muted` | that file is now dead code (see below) |
> | **#7** | No forgotten-clock-out guard anywhere in `domains/today` or the API timesheet service | deferred, acknowledged |
> | **P0-2** | No correction path after clock-out | deferred, acknowledged |
>
> **Dead code to delete, both carrying live defects:** `CoverageLanesView.tsx` (3.9 KB, no longer rendered anywhere after #33 folded coverage into the agenda) and `twoWeekRange` (`lib/localDate.ts:72`, still resolves Monday off `now.getDay()` — the P0-1 bug class, kept alive only by a test asserting it stays unused).
>
> **Constraints re-checked across the whole pass, all clean:** no `className` on any Reanimated `Animated.View`, no Tailwind `shadow-*`, no `fontFamily`, no new bare `<Modal>`, and `en`/`es` at full key parity in every namespace.
...
**Left open, deliberately:** `twoWeekRange` (`lib/localDate.ts:72`) still resolves Monday off `now.getDay()`. It is dead — its only caller was already de-fanged and `CrossFamilyRhythmView.test.ts` asserts it stays unused — but it is the same bug class sitting in the same file, and should be deleted rather than left as a trap.
...
**Still open:** there is no correction path after confirming, and no forgot-to-clock-out guard (finding #7). Those were out of scope for this fix, and #7 remains the largest exposure on the nanny side.
...
**4. The record is never shown before it's written.** The clock-out total (P0-2), time off confirming silently (#15), no forgot-to-clock-out guard (#7). Same omission three times, and thematically the worst thing here for a product whose pitch is a record two people trust.
