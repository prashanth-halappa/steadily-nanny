...
| Gap | Status | Pointer |
|---|---|---|
| **The week sent to the nanny invented hours the parent never asked for** | **Shipped** 2026-08-17 | `hydrateFromCommitments` collapsed every care-hours window on a day to `min(start)`/`max(end)` because `schedule_pattern_days` was unique on `(pattern_id, weekday)` — one block per day, structurally. A parent needing 07:00-13:00 **and** 15:00-17:00 saw **07:00-17:00 / 38 hours** on a screen whose CTA is *Send week*: two invented paid hours per Monday in the record both sides agree to. Review was also read-only while its own copy said "Change any day." — the parent lands there directly (`ScheduleBuildScreen.tsx:306`, one nanny + care hours set) and had no affordance. Fixed by migration 101 (`unique (pattern_id, weekday, start_time)`), a merge-not-union prefill (disjoint windows stay separate blocks; only overlapping/touching merge), and `WeekBlocksEditor` — inline per-slot editing on Review with per-day *Add slot* and per-slot delete, max 3 slots, overlap disables *Send week* while the wire stays permissive (#27). Materialisation needed three fixes of its own — see **GOLDEN-FIXES #46** |
