...
| Parent opened a submitted week (`parent_viewed_at`) | **No push.** A receipt is not a notification — nobody pressed anything, and buzzing the nanny that "they looked" would turn a quiet status into chatter. | The nanny Hours timeline's opened / not-opened step (`WeekTotal` `hours-status-timeline`). |
