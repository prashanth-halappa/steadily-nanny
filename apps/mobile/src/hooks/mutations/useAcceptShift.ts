/** @module hooks/mutations/useAcceptShift — carer pending → confirmed. */
...
export function useAcceptShift() {
  ...
  return useMutation<AcceptShiftResult, Error, { shiftId: string }>({
    mutationFn: ({ shiftId }) => shiftApi.accept(shiftId),
    onSuccess: data => {
      ...
      showSuccessToast(t('schedule:detail.acceptedToast'));
