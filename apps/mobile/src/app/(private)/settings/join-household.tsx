 ...
 *
 * Route: `/settings/join-household`. Reached from Settings → Household
 * (nanny / helper only). The wizard's `/onboarding/code` is unreachable the
 * moment `useIsOnboarded` says `onboarded` — `app/onboarding/_layout.tsx`
 * bounces — so a carer invited by a SECOND family had no way to redeem a
 * code, and a carer whose only membership is `removed` (reported as onboarded
 * ON PURPOSE, to keep her owed hours and pay readable) had a permanently
 * unredeemable re-invite. Same screen, settings variant: no name step, no
 * wizard step machine (see `CodeEntryScreenProps`).
 *
