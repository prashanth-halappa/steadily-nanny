...
| S3 | Paid-cancellation hint NOT in the cancel confirm dialog (sits elsewhere on the page); short-notice window configured in Manage Household while cancellation window lives in the arrangement — two homes | BUSTER | `ShiftDetailScreen.tsx:293-300` vs `:336-380` | shipped (3-T3: dialog carries the three-arm paid-hint; D-48 one window — household short-notice field readers removed from the CANCELLATION path (088 deprecation comment); no-arrangement → NOT paid, dialog says so; declined cancellation → shift stands + no-show suppressed 7d on BOTH immediate and digest legs, fails open. NOTE: `households.short_notice_hours` still gates owner-approval permissions — deliberately untouched, D-48 doesn't discuss that surface; 3-U1 inherits the Manage-Household field removal per spec §6.1). **CORRECTED at the Phase 6 persona gate: this disposition covered the PARENT half only.** The dialog fix is real, but the nanny was told nothing — the request card she answers had no pay sentence, and her standing hint was gated on a stamped flag that is false on every pre-booked shift. See R1/R2 in section G; both shipped in Phase 6. The lesson for future dispositions: "shipped" on a two-sided surface has to say WHICH side |
...
| Q2 | `reset-to-welcome.yaml` handles "onboarded" and "signed out" but not "authenticated, mid-onboarding, no household" (no tab bar → no reachable sign-out). Flows 14/15 create it on failure and strand every subsequent flow | GAP (harness) | same file | **ROOT-CAUSED AND FIXED** (Phase 5): the third arm shipped (`code-screen-sign-out`, else `onboarding-stuck-clearstate.yaml` across 8 wizard screens; all 11 testIDs verified present) but was NOT the cure, and my first reading of why was WRONG — I recorded it as "the arm can only match the screens it enumerates", i.e. a stuck mid-onboarding account. **That theory is disproved.** The real cause, found with a live probe against the wedged simulator: **the RN dev menu is TWO stacked sheets, not one.** Tapping `Continue` on the intro card does not dismiss the overlay — it ADVANCES to a second sheet (Reload / Go home / TOOLS) which keeps covering the screen. All three dismiss points in `reset-to-welcome.yaml` tapped `Continue` and assumed that closed everything, so the flow burned its whole wait budget and failed its own `assertVisible: welcome-screen` while a perfectly healthy app sat one tap underneath. The "abandoned onboarding wizard" seen in the failure frames was the WELCOME SCREEN'S OWN ARTWORK, misread from a partial screenshot. FIX: new shared `flows/dismiss-dev-menu.yaml` handling both stages, replacing all three inline copies. This single defect accounts for flows **00, 01, 05 and 15** — four of the eleven reds |

| Q3 | Flow 07: preset-button tap reports COMPLETED (dispatched, not reacted-to) but `pay-preset-sheet` never opens | GAP (harness-or-app, undetermined) | `PayTermsGroups.tsx:463`, flow 07 note | **RESOLVED — HARNESS, no app defect (Phase 6, flow 07 EXIT=0).** Phase 5's "the Save CTA stays disabled with the overtime fields correctly filled" was a symptom TWO steps downstream of the cause, and the CTA was right to be disabled every time. Three distinct harness faults, each measured from a live hierarchy dump: (1) a blind `tapOn: point: 50%,20%` "dismiss on blank space" landed on `pay-setup-preset-button`, RE-OPENING the preset sheet — proven by `pay-preset-checkbox` reading `unchecked` and `pay-preset-confirm` carrying no `enabled` attribute at failure, i.e. a fresh sheet, not the one already confirmed and closed; the sheet then hid the tree so the cancellation chip could never be found; (2) with that cleared, the chip sat at `[22,780][205,828]` while the viewport ended at 760 and the number pad occupied `[0,566][402,874]` — below the fold AND behind the keyboard, so Maestro's hierarchy-based (not occlusion-based) visibility test declared it visible, scrolled nothing, and dispatched the tap onto the '.' key. **This fully explains the "tap COMPLETED, no visible effect" class: the tap does land, just on the keyboard;** (3) neither scroll direction could recover — `UP` no-ops from offset 0%, `DOWN` overshoots to 100% and the chip leaves the a11y tree (the ScrollView drops off-viewport children). Fix: scroll UP to a known offset, dismiss the keyboard by tapping `pay-setup-backdating-hint` (a testID'd NON-interactive `<Small>`, so the tap falls through to the ScrollView — `hideKeyboard` genuinely does not work on this screen, as the file records twice), then scroll DOWN. **Root-cause guard added:** `assertNotVisible: pay-setup-cancellation-required-hint` right after the chip tap — that hint renders only while `cancellationChoice === null`, so a swallowed tap now fails THERE by name instead of resurfacing as a mysterious disabled CTA. That one missing post-condition is what cost Phase 5 the diagnosis. **Verified in the DB, not by a green tick:** rate 1500, OT 2400min@1.50, daily OT 480min, DT 720min@2.00, worked-holiday 1.50, `cancellation_paid_within_hours` NULL (the chip that was never being tapped), and `terms.preset = {common-defaults, v1, confirmed_by: <parent>}` — so the D-7 liability gate really was honoured end to end |
...
| Q5 | Flow 11: nanny's thread reply gains a stray autocorrect character and never sends; iOS QuickType's predictive bar over the send button, invisible to `maestro hierarchy` | GAP (harness) | flow 11 note | **RESOLVED (Phase 6) — flow 11 GREEN, three consecutive passes.** It is an instance of the ONE occlusion class (see the note under this table), and **three separate Phase 5 claims in flow 11's comment block are now DISPROVED**: (1) "the QuickType bar is a system overlay Maestro's RN hierarchy cannot see" is FALSE — it is `resource-id=SystemInputAssistantView` / `accessibilityText=Typing Predictions`, fully selectable, and is now guarded by name; (2) "FIXED at the simulator level" is FALSE — the three `defaults write com.apple.Preferences Keyboard*` calls run, **read back as 0, and the bar is still on screen**, so that mitigation never worked and must not be trusted; (3) "`hideKeyboard` here fires the send early" is IMPOSSIBLE — the composer is a `Textarea` (`WeekQueryThread.tsx:169`) with no `onSubmitEditing`, so the button's `onPress` is the only send path. Measured occlusion: send button `[44,561][358,609]` vs predictions bar `[0,539][402,583]` + keyboard `[0,583][402,816]` — **every pixel covered**, so the tap committed a prediction chip (the stray "Y" Phase 5 saw). **Phase 5's `hours-query-note-input is visible` failure was probably a DIRTY FIXTURE, not this bug at all:** flow 11 leaves the seed week `queried`, so a second run finds no "Ask a question" affordance. Hence the new `RESET_ONLY=1` driver mode. **The full reset rule, stated correctly (an earlier draft of this row had it half-stated and a Phase 6 driver got it backwards):** a full reseed DELETES every `pay_arrangement` for (household, nanny), and that deletion is REQUIRED, not incidental — `PaySetupScreen`'s mount effect does `router.replace('/settings/pay')` the moment an arrangement exists, so the setup form is only reachable on a carer who has none. Therefore **flow 07 NEEDS the full reseed** (it is the flow that creates the arrangement), and **08-12 must then run with NO reseed** so what 07 built survives for them to price against. The approval seeder's `--reset` is week-level only and never touches `pay_arrangements`, which is why it is the right thing to run BETWEEN those flows. Getting this backwards makes 11 look non-deterministic and silently poisons 09/12 |
...
> **THE ONE CLASS BEHIND ALMOST EVERY MAESTRO RED (Phase 6).** Nine flows were
> red across Phases 4 and 5 and were treated as nine problems. They were mostly
> **one**: *total occlusion, plus the fact that Maestro decides "visible" from the
> accessibility tree and not from what is actually on screen.* When a control
> sits under the keyboard, the typing-predictions bar or the tab bar,
> `scrollUntilVisible` declares it found and stops scrolling, and `tapOn`
> dispatches into whatever is on top. **Maestro reports COMPLETED for both.**
> That is the entire "tap COMPLETED, no visible effect" signature that cost
> Phase 5 its diagnoses — the tap is real, it just lands on the wrong element.
> Measured, each from a live hierarchy dump:
> `07` chip `[22,780][205,828]` vs keyboard `[0,566][402,874]` → hit the `.` key ·
> `11` send `[44,561][358,609]` vs predictions bar `[0,539][402,583]` + keyboard
> `[0,583][402,816]` → every pixel covered · `13a` `settings-sign-out`
> `[22,818][380,866]` vs TAB BAR `[0,791][402,874]` → centre `(201,842)` is a
> tab-bar hit · `14` `code-screen-cta` `[22,760][380,808]` ENTIRELY behind the
> keyboard `[0,539][402,874]`. **Flow 14 carries the strongest proof of the
> whole class, and it is negative evidence: the API log contains ZERO
> `invites/:code/preview` lines across every failing run — the request was never
> issued, so `code-preview-card` (gated on `preview.data`,
> `CodeEntryScreen.tsx:366`) never had a chance to render.** Nothing about the
> app was ever wrong. Note also why the earlier `swipe` fix could not have
> worked: that CTA is pinned OUTSIDE the scroll body (`[0,172][402,760]`), so
> scrolling can never move it out from under the keyboard — the keyboard has to
> actually go. It is dismissed by tapping the screen title, deliberately NOT the
> string "Invite code", which is also `code-input`'s own accessibilityLabel and
> would merely refocus the field. The fix is to dismiss the keyboard by tapping non-interactive
> content, and then **the right scroll option, which depends on the CONTAINER —
> `centerElement: true` is NOT a general remedy and an earlier draft of this
> note wrongly said it was:**
> - **Container ENDS at the tab bar** (e.g. `hours-week-list`, bounded
>   `[0,63][402,791]`, and the tab bar starts at 791): Maestro's default
>   `visibilityPercentage: 100` already means "fully clear of the tab bar", so a
>   PLAIN `scrollUntilVisible` is correct. Adding `centerElement` here actively
>   BREAKS it — a LAST element can never reach the viewport centre because there
>   is nothing below to scroll against, so it fails "No visible element found"
>   on an element plainly on screen (measured: `hours-export-button` reported
>   not-found at `[22,629][380,677]`, fully visible and tappable).
> - **Container extends UNDER the tab bar** (e.g. the Settings scroll view, so
>   `logout.yaml`): 100%-visible is satisfiable while still occluded, so
>   `centerElement: true` is genuinely required.
>
> Also iOS-specific: **`- back` is an edge-swipe NAVIGATION gesture and does not
> dismiss a `BottomSheetBase`** — tap `bottom-sheet-close-button` instead, or the
> sheet stays up and the flow dies on a later assertion.
>
> **AND IT CUTS BOTH WAYS — the deepest finding of the phase.** Maestro also
> reports things NOT visible that are plainly on screen. The dev-menu sheet that
> blocked the entire suite for a day was finally traced not to the tap but to
> the GUARD: `tapOn: id: 'xmark'` dismisses it 100% of the time, while
> `when: visible: text: 'Runtime version'` evaluated FALSE with the sheet on
> screen and that exact string in the hierarchy — so the dismiss block never
> ran. Looping the dismissal does not help; it evaluates the broken guard N
> times. The fix was a DELETION: an unconditional `tapOn: {id: xmark, optional:
> true}`, which is already a no-op when the sheet is absent. **So: prefer an
> unconditional `optional:` action over a `when: visible:` guard wherever the
> action is harmless when the element is missing, and never treat a visibility
> predicate as evidence about the screen.** Four theories died before this one
> (two stacked sheets; Continue-vs-xmark; `EXDevMenuIsOnboardingFinished`
> suppression, which is NOT the gate — the key persists and the sheet still
> appears; and timing/bounded waits).
>
> **The lesson for anyone reading a future red: an assertion
> that fails several steps after a COMPLETED tap is an occlusion suspect first
> and an app defect second.** `13a` in particular was recorded by Phase 5 as
> "a third unfixed gap in the reset path"; `settings-sign-out` is an
> unconditional top-level button (`settings.tsx:516`) that was never missing,
> and because it lives in `logout.yaml` — which every flow's reset routes
> through — it was also causing sporadic `welcome-screen is visible` failures
> blamed on whichever flow happened to call it.

### G. Found during Phase 6 ship (2026-08-12)

Found by the final Marisol/David persona gate and by the orchestrator's own
verification of their claims. Added so §0.1's completeness contract still holds:
R3 in particular existed only as a clause of prose in §2c and had **no id at
all**, which is precisely the hole the contract exists to close.

| # | Obs | Cat | Anchor | Disposition |
|---|---|---|---|---|
| R1 | The cancel-request card the NANNY answers carries no pay sentence at all. `cancelPaySentence` has exactly two consumers — the parent's confirm dialog (`ShiftDetailScreen.tsx:333`) and the standing hint (`:406`) — and the change-request block (`:652-760`) is neither. So the person whose pay is at stake is the only party not told | BUSTER | `ShiftDetailScreen.tsx:652-760` | shipped (Phase 6, owner-approved off Marisol's must-fix): the existing sentence added to the request card. S3/D-48 had shipped its PARENT half only; §2b's `shipped` on S3 overstated it and this row records the carer half |
| R2 | The standing cancellation-pay hint is gated on `shift.is_short_notice`, stamped when the shift is authored — against `cancellationPay.ts`'s own module header ("a TIMING fact, never a pay claim, so it does not gate anything in this module") AND the same file's comment at `:339-345` ("a shift booked three weeks out still reads false on the morning it starts"). The co-parent gate 60 lines away computes it LIVE; this does not. For a full-time nanny on a fixed schedule the flag is false on every shift, so the hint never fires for the person it exists for | BUSTER | `ShiftDetailScreen.tsx:406` | shipped (Phase 6, owner-approved): gate recomputed live from `short_notice_hours`, matching the co-parent gate's existing pattern |
| R3 | The shift Day thread is a debug log, not the evidence record Marisol's §2c ask names ("schedule-change timestamps as evidence"). `EventRow` renders no actor and no date (`formatInstantDisplay` requests hour+minute only), and `KNOWN_EVENT_TYPES` covers just 3 of the event types (`shift_updated`, `pattern_conflict`, `uncovered_care`) so everything else falls to "Update" + the raw literal `Event: shift_confirmed`. **Had no gap id anywhere** — not §2b, not `docs/DEFECT-LOG.md` (D24/D35 are unrelated) | GAP | `ShiftDetailScreen.tsx:880-915`, `KNOWN_EVENT_TYPES` at `:93-97` | **deferred (owner-signed, Phase 6)** — id assigned now, fix after release. Cost is SMALLER than the review assumed: `shift_events.actor_id` already exists in the DB **and** on the wire (`ShiftEventSchema.actor_id` at `shift.schema.ts:398`) and is simply unread by `EventRow`, and `ShiftDetailScreen` already has a `nameFor()` helper — so this is UI + i18n, **no migration**. Revisit **2026-09-15**, first non-freeze window |
| R4 | `en/pay.json:144` + `es/pay.json:144` promise "Steadily shows them on the week but never adds them to the week's gross" — but D-54 deferred exactly that week display. The app states a promise at terms-entry that it does not keep, and the nanny is who finds out | BUSTER (small) | `pay.json:144` both locales | shipped (Phase 6, owner-approved): the week claim removed from both strings. D-54 stands; the copy now matches it |
| R5 | The overtime fields accept any values — "after 60 hours at 1.1×" draws no objection — against Marisol's Phase 2 condition that presets encode the law rather than what families wish it were. In tension with D-7/D-44/D-52, which deliberately keep the app from owning a legal conclusion | GAP | pay-terms form (`PayTermsGroups.tsx`) | shipped (Phase 6, owner-approved): a NON-BLOCKING inline caution above 40h or below 1.5×, naming the common US values and leaving local-law responsibility on the family per D-7. Refuses nothing, blocks nothing — a caution is not a legal conclusion, and the form already says a comparable hard thing about biweekly averaging building a wage claim |
| R7 | **The onboarding layout unmounts the whole wizard Stack on ANY transient `loading`, destroying every wizard screen's local state.** `onboarding/_layout.tsx:58-61`: `showWizard` is true only for `status === 'not-onboarded'` OR (`'onboarded'` AND `wizardEngaged`) — **`'loading'` matches neither**, so the `Stack` unmounts and the spinner shell renders. Any mutation that invalidates `queryKeys.user.memberships()` mid-wizard therefore flips `useIsOnboarded` to `loading` and tears down the screen the user is on; expo-router then restores the route with fresh `useState`. This is the mechanism behind R8's dead-end, found only by tracing why the code field cleared while the name field survived | GAP (latent) | `apps/mobile/src/app/onboarding/_layout.tsx:58-61` | **documented, NOT fixed (Phase 6).** The guard is DELIBERATE — its own comment says "loading, onboarded-and-not-engaged, or membershipsError — never paint RoleScreen… here we just wait" — and the author plainly expected a brief wait, not a state-destroying unmount. Changing a guard that gates the entire onboarding stack during a freeze is the wrong trade, and R8's fix removes the only known trigger AND makes the affected screen survive it. **Residual risk:** any OTHER wizard screen holding local state (household name, children names/ages) would lose it if some future path invalidates memberships while it is open, with no recovery of its own. Real fix: treat `loading` as "keep rendering what is already mounted" when `wizardEngaged`. Revisit **2026-09-15**, first non-freeze window |
| R8 | Post-redeem wizard dead-end: after a fully successful join (`POST /invites/redeem` 200, membership active, draft household live) the wizard stays on the CODE step with the field cleared, and the consumed code 404s on retry. Root cause is a THREE-step chain: TanStack calls `useRedeemInvite.onSuccess` **before** `mutateAsync` resolves → the immediate membership invalidation runs before `setRole`/`setCurrentStep` can persist → `useIsOnboarded` goes `loading` → R7's guard unmounts the Stack → remount restores CODE with fresh local `useState` (name survives because it is store-backed; `code` does not — **that asymmetry is what identified the re-mount**) | BUSTER | `CodeEntryScreen.tsx`, `useRedeemInvite.ts` | shipped (Phase 6, owner-approved): defence in depth — (1) membership invalidation deferred via `queueMicrotask` so the step persists first, removing the trigger; (2) `router.replace` plus on-mount recovery — if the persisted `currentStep` is already past CODE, the screen re-navigates and shows a "finishing setup" shell instead of an empty code form, so the dead-end is survivable even if the ordering ever changes; (3) the earlier `lockInvitePreview` + `post-redeem-error` affordances kept. **Two earlier hypotheses were killed with evidence and are recorded so nobody re-derives them:** it was NOT the D-55 missing-route class (`onboarding/notifications.tsx` exists), and NOT a swallowed error (`post-redeem-error` never rendered, so nothing threw). It was also NOT a stale Metro bundle — proven by BEHAVIOUR, since the post-redeem 404 refetch disappeared, which can only happen if the preview lock is live |
| R6 | Parent-created households have **no terms acceptance gate**: the parent types the terms, the nanny taps "I've seen these terms", and she is working under terms she never agreed to. Survivable only because the record says exactly that — "Seen", never "Agreed", on both screens (D-41) — and inverted in the nanny-first direction, where D-35 makes parent acceptance the binding act | GAP (by design) | D-31/D-35/D-41 | **documented (Phase 6)** — named at Marisol's explicit request rather than carried quietly. Not a defect: D-41's two-words-two-acts split is what makes it honest. Revisit only if a parent-side acceptance act is ever wanted |
...
> phase 5 artifact: https://claude.ai/code/artifact/19d4d5d3-65d6-4d37-adc9-a0b9965bb04c
---
...
|---|---|---|---|
| Phase 6 (ship) | 2026-08-12 | **Prod ship sequence CLOSED** (migrations → wipe → API → D-37 worker → banner sweep). Maestro §0.2 was already green (17/17) earlier the same day. Store submit remains held (`docs/STORE-SUBMISSION-PACK.md`). | **(1) Migrations 074→096** applied via Supabase MCP `apply_migration` on prod `dylhrlvfkibipdkguptz`, in order (skipped missing 083/084/091). Tip: `redeem_draft_invite_week_starts_on`. Cron jobs verified present: `cover-ask-expiry` (`3-58/5 * * * *`), `shift-completion` (`40 3 * * *`), `no-show-digest` (`50 * * * *`) plus the pre-existing six. **(2) D-9 wipe executed and verified** — deleted all `public.households` then all `auth.users`; truncated `job_runs` + `push_reminder_log`. Pre-wipe: 10 users / 7 households / 302 shifts. Post-wipe: all zero; `app_config` retained (1 row). **(3) API Cloud Run** redeployed after migrations (`nanny-api-00002-cf5`, 100% traffic). Gateway health `https://api.nanny.getsteadily.app/health` → 200 on the new revision (uptime ~46s at check). Ordering held: 088 DDL was live before this revision (which writes `cover_ask_expires_at`). **(4) D-37 CF worker** `nanny-getsteadily` deployed (version `77742b1e…`) with `API_BASE_URL=https://api.nanny.getsteadily.app` + `POSTHOG_API_KEY` secret; `/t/ZZZ-ZZZ` returns opaque 404; site home 200. **(5) Banner sweep:** 11 `REPO FILE ONLY` headers on 080–096 replaced with `APPLIED TO PROD 2026-08-12` notes. **Deferred (owner-signed) with revisit dates:** D-10/S7 timezone-move (revisit on first household timezone change support ask); D-30 receipt photos (revisit when reimbursement volume proves need); D-43 CA duties/classification (revisit only if owner reopens legalese); D-54 week “outside wages” section (revisit **2026-09-15** with R3); D-57 concurrency pair (revisit **2026-09-15**, FOR UPDATE RPCs); R3 Day-thread evidence UI (revisit **2026-09-15**); R7 onboarding-layout unmount-on-loading (documented latent; revisit with next onboarding touch). **Held / not this row:** EAS store submit; `min_supported_version` pin at first store build; Android universal-link SHA-256 until Play signing fingerprint. Post-ship watch: `docs/POST-SHIP-WATCH.md` + `docs/ROLLBACK-RUNBOOK.md` (D-58: no Sentry — integrity-checks → API logs → PostHog funnel → cron volumes). |
| Playbook authored | 2026-08-10 | this file @ `main` 9459d9e | — |
| Phase 5 (integration freeze) | 2026-08-12 | **PARTIAL — see the Maestro caveat.** Shipped on `phase5-integration-freeze` (7 commits `79a7afd`..`5b730db`, 141 files, +4457/-714), NOT pushed, working tree clean. Merge state verified: all 19 wave branches 0 commits ahead of main. **qc FULLY GREEN on the committed state: 8,338 tests / 0 fail** (mobile 3855, api 3833, st 621, scripts 29), lint/format/typecheck clean. **§2b REGISTER CLOSED** — 81 rows parsed with an escaped-pipe-aware parser; every row terminal except the Maestro-gated harness rows (Q1-Q5). 18 PRESERVE rows → preserved-verified against Phase 4's gate 4; T8 → deferred (D-54); T14/T17/Q6/Q7/Q8/Q9/Q10/Q11 closed with evidence. **THREE defects the suite was actively hiding.** (1) `TermsProposalCommandService`'s `candidates` dep defaulted to `null` AND the production singleton used that default, so `activateCandidate` was a silent no-op in prod — every nanny-first/absorption acceptance would have failed at `assertActiveNanny`; tests passed only because they injected a fake (the D53 trap exactly). Its own TODO(3-O) prescribed the fix and was never actioned after the dependency merged; the new test builds with the REAL default and spies on the repo prototype. (2) `SETUP_STEP_ROUTES.TERMS` named `/onboarding/terms`, a route with NO file — nanny → create → CTA dead-ended on `+not-found`, making D-33/D-37/D-38's whole acquisition loop unreachable in-app (D-55; moved to `app/(private)/draft/terms.tsx`, deliberately outside `/onboarding`, whose layout bounces an already-onboarded user). (3) Q7's redaction had a FOURTH leak: the regex required a trailing slash, so a bare 404 probe still logged the bearer code — and morgan logs 404s. **D-56: all four owed Today surfaces shipped under an explicit hierarchy** — `terms_proposal` had ZERO owners (filtered from NeedsAttentionCard for a card nobody built, with a dead `termsProposal` rung in attentionOwner.ts). Caps enforced via the EXISTING `demoted` mechanic: parent ≤3 attention cards, nanny ≤2. **Cross-track seams**: 6 real coverage gaps found and fixed, clustered on `paid_holiday` across breakdown/one-liner/PDF/CSV plus the fully-reversed export case; all 55 push types verified routed; SEAM 3 (terms-ack → week explainer) honestly reported **NOT WIRED** and accepted — the ack lives only in the pay domain by D-31/D-41 design. **Q8 root-caused**: global `afterEach(cleanup)`; only 3 files broke (threshold 10) and BOTH 15s ParentWeekView band-aids were REMOVED and now pass at the default 1000ms — proof the flake class was fixed, not moved. **Q9**: 60 files hardcoded Aug-2026 dates (not ~12); 53 converted, 6 correctly left absolute for pinned DST/weekday semantics. **i18n**: 1647 keys en↔es clean, one real untranslated value fixed. | **Migration rehearsal — Supabase branching needs the Pro plan, so it ran LOCALLY and stronger.** A scratch DB rebuilt to prod's exact 073 state (auth schema dumped from the live local DB; pg_cron/pg_net/vault stubbed since they cannot exist outside the `postgres` DB — so **cron contents must still be verified against prod in Phase 6**, the 054 lesson), seeded with rows verified against real prod (Monday dow=1, GBP, an approved timesheet whose frozen snapshot carries **NO `v` key** — exactly 1-A's case), then **074→096 applied IN ORDER, all 20 clean**; `run_integrity_checks()` clean; snapshot md5 IDENTICAL afterwards, `week_starts_on` still 1, `payments.kind` defaulted to 'payment'. **FINDING: 074 sets `households.currency not null default 'USD'` while existing arrangements/timesheets stay GBP** (074's header admits it) — moot under D-9's wipe, but backfill from the household's own arrangements if the wipe is ever skipped. **Old-client drill: the premise does not hold — the app has never launched, so the shipped fleet is the EMPTY SET** (owner-confirmed; prod's 7 households are his test data). `min_supported_version` is NULL, read as `?? '0.0.0'`; recommended posture is pinning it to the first store build, closing all six wire-breaking items at once. **New `docs/ROLLBACK-RUNBOOK.md`** — central finding: `app_config` has NO generic feature-flag surface, so `cron.unschedule` is the real kill switch (`cover-ask-expiry` at */5 = highest blast radius), and every new money rule is null-gated per household so clearing a term is a new append-only row. Records the **asymmetric deploy order**: 088 carries DDL and `shiftRepository` writes `cover_ask_expires_at` unconditionally, so deploying the server before 088 makes EVERY shift creation 500. **D-57**: two concurrency defects ACCEPTED (duplicate arrangements on concurrent accept; non-atomic reimbursement sum/insert) — both need a `FOR UPDATE` DB function in 077's shape, too large for a freeze; symptoms in runbook §9. **D-58**: Sentry disabled, so §11's triage is replaced by integrity-checks → API logs → PostHog funnel → cron volumes. **Item 5 closed by owner** — no EAS builds ("stick to dev build"); note `production` is store-distribution (iOS needs TestFlight, Android .aab cannot be sideloaded) and `EXPO_PUBLIC_SENTRY_DSN` is still the literal `TODO-SET-BEFORE-BUILD`. **MAESTRO — TWO FULL 00-15 PASSES + 3 STANDALONE RE-RUNS, ~2h40m. FINAL AUTHORITATIVE MATRIX: 8 PASS / 9 FAIL. §0.2's E2E GATE IS NOT MET — carried to Phase 6.** GREEN: 00, 01, 03, 04, 05, 06, 10, 13b. RED: 02, 07, 08, 09, 11, 12, 13a, 14, 15. Most of the elapsed time was environment, not flows. **THREE harness bugs found and FIXED (harness YAML only, zero app code):** (1) **the RN dev menu is TWO stacked sheets** — tapping `Continue` on the intro card only ADVANCES to a second Reload/Go-Home/Tools sheet that keeps covering the screen; all three dismiss points in `reset-to-welcome.yaml` assumed one sheet. Proven with a live probe: `Continue` reported COMPLETED, landed on sheet two, and one more tap revealed the app had been correctly on the welcome screen the whole time. New shared `flows/dismiss-dev-menu.yaml` handles both stages. **This alone unblocked 00, 01, 03 and 05.** Critically, the sheet HIDES THE A11Y TREE, so it also corrupts `reset-to-welcome`'s own visibility guards — the launcher arm then fires wrongly and force-reloads the app mid-flow. (2) **`select-household.yaml` tapped `tab-settings` unguarded**, which a household-less user mid-onboarding does not have — exposed by the Q1 fix that made `login.yaml` call it unconditionally; now guarded, and flow 14 progresses far past it. (3) **flow 08 needed `scrollUntilVisible`** before the sick-badge assertion (the row existed but sat below the fold), matching a pattern the same file already used 3 lines earlier. **REMAINING REDS, classified:** 09 and 12 are pure collateral of 07 and 11 (each flow's own comments say so). 08 was ENVIRONMENT — `PHASE4_TODAY_SHIFT_ID` came back EMPTY; **root cause measured, and both first theories were wrong**: not a stale calendar date (the roll-forward logged a no-op `2026-08-12 -> 2026-08-12`) but TWO `kind='extra'` shifts on today, where `.maybeSingle()` ERRORS on multiple rows and the call destructured only `{ data }`, swallowing it — an empty id then flowed into a deep link as a valid-looking URL. Fixed in `seed-phase4-fixtures.ts` (`0e8e526`); the file had already documented that same `.maybeSingle()` hazard elsewhere and simply not applied it. **SWEPT THE CLASS, not just the instance: 18 of 21 `maybeSingle()` calls across `scripts/` discard the error the same way.** Rather than 18 edits, ONE guard now sits at the driver-contract boundary and refuses to emit any empty/undefined/null value, naming the offending key — which covers every existing lookup and every future one without anyone remembering. The dangerous shape is specifically: swallowed Supabase error → empty string → interpolated into a deep link as a valid-looking URL → flow dies several steps later on an unrelated assertion. 07 narrowed but STILL UNDETERMINED — now fails one step later at `pay-setup-screen-cta, enabled`: the Save button renders but stays disabled with overtime fields correctly filled, matching the flow's own note that the CTA "stays disabled without a choice" of cancellation policy. 13a is a THIRD, unfixed gap in the reset path (`settings-sign-out` not found on a live session). 14 now fails later at `code-preview-card`; 15 still cascades from 14. **02 and 11 hit a genuinely OPEN, low-frequency class: the app disappears mid-relaunch** — `App crashed or stopped`, NO `.ips`, failure frame on the launch splash. It moved between flows across runs and survived the dev-menu fix, so it belongs to the `launchApp: clearState: false` relaunch path, not to any flow's content. **FREQUENCY MEASURED: 3 occurrences across the whole session — 2 in the single clean 00-15 pass (~17-18 relaunches), plus 1 earlier on flow 03 — always a different flow, always immediately after the previous flow's teardown, always with no `.ips`. That is roughly a 1-in-9 relaunch failure rate, which is high enough to matter and low enough that a single passing re-run proves nothing.** A mechanism was proposed and is NOT proven; 03 passing once was taken as confirmation and should not have been.Harness lessons:** per-flow `tee` logs sit at 0 bytes because the Maestro JVM block-buffers piped stdout — "log not growing" is NOT a stall signal and acting on it kills healthy flows; use `~/.maestro/tests/<ts>/maestro.log` mtime. `config.yaml`'s `screenshotDirectory` is IGNORED (every PNG lands at `.maestro/` root). And the driver needs `MAESTRO_DRIVER_STARTUP_TIMEOUT=120000` after any kill sweep. **Phase 6 priorities, in order: (a) 13a's reset sign-out path, (b) live interactive probing of 07 and 14 — both show "tap COMPLETED, no visible effect" and may share a root cause, (c) bound the frequency of the relaunch class before deciding whether it needs a retry wrapper or an investigation.** |
