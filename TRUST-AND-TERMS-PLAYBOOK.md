...
| # | Obs | Cat | Anchor | Disposition |
|---|---|---|---|---|
| Q1 | E2E reset does not normalise the ACTIVE HOUSEHOLD, so a switch left by flow 10 leaks into every later flow — the same Hours deep link then resolves to the Sunday-anchored week and the assertion fails against a correct app (proven from the failure's own `hierarchyRoot`) | GAP (harness) | `flows/reset-to-welcome.yaml`; `flows/select-household.yaml` exists unused | OPEN — one-place fix, clears flows 04+05 |
| Q2 | `reset-to-welcome.yaml` handles "onboarded" and "signed out" but not "authenticated, mid-onboarding, no household" (no tab bar → no reachable sign-out). Flows 14/15 create it on failure and strand every subsequent flow | GAP (harness) | same file | OPEN — one-place fix, clears the 14/15 cascade |
| Q3 | Flow 07: preset-button tap reports COMPLETED (dispatched, not reacted-to) but `pay-preset-sheet` never opens | GAP (harness-or-app, undetermined) | `PayTermsGroups.tsx:463`, flow 07 note | OPEN — needs an interactive `maestro studio` session |
| Q4 | Flow 08: parent's Changes card shows neither accept/decline nor withdraw, though the DB proves the request is `pending` with a non-null requester and those are the only gating conditions | GAP (reads as visibility/scroll) | `ShiftDetailScreen.tsx:662,721,733` | OPEN — 09/12 are downstream of Q3/Q5, not separate |
| Q5 | Flow 11: nanny's thread reply gains a stray autocorrect character and never sends; iOS QuickType's predictive bar over the send button, invisible to `maestro hierarchy` | GAP (harness) | flow 11 note | OPEN — disable prediction on the sim; record it as a harness step |
| Q6 | Invite codes generated with `Math.random()`, not a CSPRNG | GAP (security) | `inviteCode.ts:23` | OPEN — deliberately not changed; own slice (alters generation for existing rows) |
| Q7 | The invite code — a bearer secret — still reaches logs via morgan's URL, `logError`'s `req.path`, and `InviteNotFoundError.metadata.identifier`; only one copy was removed | BUSTER (privacy) | `publicInviteRoutes`, `errorHandler` | OPEN — real fix is path redaction for that route prefix |
| Q8 | `useRecordPayment`'s two `invalidateQueries` calls cause a refetch cascade that outruns testing-library's 1000ms default — band-aided a THIRD time | GAP (test harness) | `useRecordPayment.ts`; `ParentWeekView.payments.test.tsx` | OPEN — root cause is the missing `afterEach(cleanup)` in mobile setup |
| Q9 | ~12 test files hardcode mid-August-2026 dates; two went red mid-session when real UTC crossed 2026-08-12 | GAP (hygiene) | `grep -rln "2026-08-1[0-9]T" apps/api/tests` | partially fixed (2 files derive from `Date.now()`); rest latent |
| Q10 | `reimbursementSettlementService` and `termsProposalCommandService` state machines never traced end to end — the only money paths the review did not cover at depth | GAP (review coverage) | both services | OPEN — finish in Phase 5 |
| Q11 | `EarningsBreakdownSheet`'s overtime row uses a non-indexed testID, so two overtime lines produce duplicate testIDs | GAP (minor) | `EarningsBreakdownSheet.tsx` | OPEN — pre-existing; harmless to render, breaks E2E selection |
