...
| [`docs/06-MOBILE-ARCHITECTURE.md`](./docs/06-MOBILE-ARCHITECTURE.md) / [`docs/design/00-FOUNDATIONS.md`](./docs/design/00-FOUNDATIONS.md) | Before touching `apps/mobile` |
| [`docs/design/README.md`](./docs/design/README.md) | The design doc set — tokens, the L1–L4 hierarchy rules, voice, art direction, and the per-screen specs — before touching any mobile UI |
...
**Tests** (per `docs/09-TESTING.md`): `apps/api` tests are NOT colocated with source — they live under `apps/api/tests/unit/domains/<feature>/{services,controllers,routes,repositories,utils}/`, mirroring the source tree one directory over (e.g. `apps/api/tests/unit/domains/pay/services/*.test.ts`). The DB integration tier lives under `apps/api/tests/integration/` (run with `bun run test:db`). Mobile component tests ARE colocated, under a `__tests__/` folder next to the component — **never** a `*.test.ts(x)` file colocated directly inside `src/app/` next to a route file (expo-router will try to treat it as a route — see `GOLDEN-FIXES.md` #8).

## Critical mobile gotchas

- **Household context vs entity context:** Never assume `useActiveHousehold()` matches the household context of the entity you are rendering. Check that the two agree or derive formatting/timezone/currency directly from the entity, to avoid rendering the wrong family's context over the right family's data.
- **Ungated queries stating facts:** Never let an unhandled `isLoading` or `isError` branch fall through into rendering a factual assertion (e.g., "Paid £0.00" on a dropped connection, which invites a double payment). Degrade gracefully to a neutral empty or skeleton state on failure.
