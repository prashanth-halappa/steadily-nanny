...
- **Sora font weights go through `fontFamily`, not numeric `fontWeight`.** Each weight is a separate font file (`Sora-Bold.ttf`, etc.); a numeric `fontWeight` is a no-op or faux-bold on iOS. Always pair a `weight` value with the matching `fontFamily: 'Sora-*'` string — see `apps/mobile/lib/design-tokens/typography.ts`. (`GOLDEN-FIXES.md` #3.)
