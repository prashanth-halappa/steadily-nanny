...
- **Ledger uses the platform face — weight via `fontWeight`, not `fontFamily`.** Omit `fontFamily` (SF Pro / Roboto). Typography tokens and Tailwind (`font-medium`, etc.) set numeric weight. Do not set `fontFamily: 'System'`. See `apps/mobile/lib/design-tokens/typography.ts` and `GOLDEN-FIXES.md` #3.
